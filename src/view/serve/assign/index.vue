<template>
    <div class="mod-config">
            ...
            <el-table-column fixed="right" header-align="center" align="center" width="150" label="操作">
                <template slot-scope="scope">
                    <el-button type="primary" size="small" @click="addOrUpdateHandle(scope.row.id)"
                        v-hasPermi="['serve:assign:do']">分配</el-button>
                </template>
            </el-table-column>
        </el-table>
        ...
        <assignProceFeed v-if="assignProceFeedVisible" ref="assignProceFeed" @refreshDataList="getDataList">
        </assignProceFeed>
    </div>
</template>
  
<script>
import assignProceFeed from './assign-proce-feed.vue'
...
export default {
    ...
    methods: {
        ...
        addOrUpdateHandle(id) {
            this.assignProceFeedVisible = true
            this.$nextTick(() => {
                this.$refs.assignProceFeed.init(id)
            })
        }
    }
}
