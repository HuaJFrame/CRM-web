  ...
  <el-dialog title="服务分配" :close-on-click-modal="false" :visible.sync="visible">
    <el-form :model="dataForm" :rules="dataRule" ref="dataForm" @keyup.enter.native="dataFormSubmit()" label-width="80px">
      ...
      <el-form-item label="服务处理" prop="serviceProce">
        <el-input v-model="dataForm.serviceProce" placeholder="服务处理" disabled></el-input>
      </el-form-item>
      <el-form-item label="处理结果" prop="serviceProceResult">
        <el-input v-model="dataForm.serviceProceResult" placeholder="处理结果"></el-input>
      </el-form-item>
    ...
    init(id) {
      ...
      this.$nextTick(async () => {
        ...
        if (data.code === 200) {
          ...
          this.dataForm.serviceProce = data.customerServe.serviceProce
          this.dataForm.serviceProcePeople = data.customerServe.serviceProcePeople
        }
      })
    },
    // 表单提交
      ...
      this.$refs['dataForm'].validate(async (valid) => {
        if (valid) {
          let { data } = await request.post("serve/save", {
            ...
            'serviceProceResult': this.dataForm.serviceProceResult,
